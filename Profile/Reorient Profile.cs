...
namespace IEF_Toolbox.Profile
{
    public class DieProfilePrep : GH_Component
    {
        ...
        {
            pManager.AddTextParameter("string_Referenced Block Name/Profile ID", "ID", "Name of the die profile", GH_ParamAccess.item);
            pManager.AddBooleanParameter("bool_Bake", "B", "Set to True to bake the reoriented Curve to the Rhino Space", GH_ParamAccess.item);
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            //GET OBJECT BY KEY(IEF_NUMBER) VALUE(BLOCKNAME) BY IEF_NUMBER STRING
            RhinoObject uB = null;
            var uBs = RhinoDocument.Objects.FindByUserString("IEF_NUMBER", BlockName, false);
            if (uBs.Length > 0){ uB = uBs[0]; }
            else { AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "No name block is found for "+BlockName); }

            if (uB!= null) { 
                profile.ProfileDescription = uB.Attributes.GetUserString("PART_NAME:");
                profile.Material = uB.Attributes.GetUserString("MATERIAL:");
                profile.Finish = uB.Attributes.GetUserString("FINISH:");

                profile.DetermineProfileTypeFromProfileDescription();
            }
            ...
            // mirror/move the profile if the profile type is chickenhead, horizontal receptor, vertical blade and vertical receptor
            string[] selList = { "CHICKEN HEAD", "HORIZONTAL RECEPTOR", "VERTICAL BLADE", "VERTICAL RECEPTOR" };

            if (selList.Contains(profile.ProfileType)) {
                PolylineCurve seg = profile.OutsideCrv.ToPolyline(0,0,0,double.MaxValue);
                Curve[] segmCrv = seg.DuplicateSegments();
                double yOffset = segmCrv.OrderBy(crv => crv.GetLength()).ToArray()[segmCrv.Length - 1].PointAtLength(0).Y;
                profile.ProfileCrv = profile.ProfileCrv.Where(crv => crv.Translate(new Vector3d(0, -yOffset, 0))).ToList();
                profile.SortInsideOutside();
                if (profile.ProfileType == "VERTICAL BLADE" || profile.ProfileType == "VERTICAL RECEPTOR") {
                    if (profile.OutsideCrv.GetBoundingBox(false).Center.Y < 0)
                    {
                        Transform mirror = Transform.Mirror(new Plane(new Point3d(0, 0, 0), new Vector3d(1, 0, 0), new Vector3d(0, 0, 1)));
                        profile.ProfileCrv = profile.ProfileCrv.Where(crv => crv.Transform(mirror)).ToList();
                        profile.SortInsideOutside();
                    }
                }
            }
            ...
            if (success3)
            {
                if (_Bake) {
                    ...
                    profile.VersionNumber = newVersion;
                    ...
                    att.SetUserString("Profile ID", profile.ProfileID);
                    att.SetUserString("Profile Type", profile.ProfileType);
                    att.SetUserString("Profile Description", profile.ProfileDescription);
                    att.SetUserString("Material", profile.Material);
                    att.SetUserString("Finish", profile.Finish);
                    att.SetUserString("Bake Time", bakeTime);
                    att.SetUserString("Bake Version", newVersion.ToString());
                    att.SetUserString("Unique ID", profile.uniqueID);
                    att.SetUserString("Area", profile.Area.ToString());
            ...
            // get the block information in the base block
            void GetBaseBlockAttributes (string profileID, out string partType, out string material, out string finish) {
                string pT = null;
                string m = null;
                string f = null;
